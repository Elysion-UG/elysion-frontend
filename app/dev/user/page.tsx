  ...
  return (
    <Card className="mb-4">
      ...
      <CardContent className="space-y-3">
        ...
        {response !== null && (
          <pre
            className={`mt-2 max-h-60 overflow-auto whitespace-pre-wrap rounded border p-3 text-xs ${isError ? "border-red-200 bg-red-50 text-red-700" : "border-slate-200 bg-slate-50"}`}
          >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <h1 className="mb-1 text-2xl font-bold text-slate-800">User & Profile API Test</h1>
        <p className="mb-6 text-sm text-slate-500">
          User info, value profile, addresses, seller profile
        </p>
        ...
        {/* User Me */}
        <h2 className="mb-3 mt-6 text-lg font-semibold text-slate-700">User Me</h2>
        ...
        >
          <div>
            ...
            <Input placeholder="DELETE" className="h-8 border-red-200 text-xs" />
          </div>
        </EndpointCard>

        {/* Buyer Value Profile */}
        <h2 className="mb-3 mt-8 text-lg font-semibold text-slate-700">Buyer Value Profile</h2>
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() => {
              let sp, ep
              try {
                sp = JSON.parse(simpleProfile)
              } catch {
                sp = {}
              }
              try {
                ep = JSON.parse(extendedProfile)
              } catch {
                ep = {}
              }
        ...
        >
          <div className="space-y-2">
            ...
            <div>
              <Label className="text-xs">simpleProfile (JSON)</Label>
              <Textarea
                value={simpleProfile}
                onChange={(e) => setSimpleProfile(e.target.value)}
                className="h-20 text-xs"
              />
            </div>
            <div>
              <Label className="text-xs">extendedProfile (JSON)</Label>
              <Textarea
                value={extendedProfile}
                onChange={(e) => setExtendedProfile(e.target.value)}
                className="h-20 text-xs"
              />
            </div>
          </div>
        ...
        {/* Addresses */}
        <h2 className="mb-3 mt-8 text-lg font-semibold text-slate-700">Addresses</h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex items-end gap-2 pb-1">
              <Checkbox
                id="addr-default"
                checked={addrDefault}
                onCheckedChange={(v) => setAddrDefault(!!v)}
              />
              <Label htmlFor="addr-default" className="text-xs">
                Is Default
              </Label>
            </div>
          </div>
        ...
        <Card className="mb-4 border-slate-200">
          <CardContent className="pt-4">
            <Label className="text-xs font-semibold">
              Address ID (for PATCH / DELETE / set-default)
            </Label>
            <Input
              value={addrId}
              onChange={(e) => setAddrId(e.target.value)}
              placeholder="UUID of the address"
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() =>
              apiRequest(`/api/v1/users/me/addresses/${addrId}`, {
                method: "PATCH",
                body: JSON.stringify({
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs">Street</Label>
              <Input
                value={patchAddrStreet}
                onChange={(e) => setPatchAddrStreet(e.target.value)}
                className="h-8 text-xs"
              />
            </div>
            <div>
              ...
              <Input
                value={patchAddrCity}
                onChange={(e) => setPatchAddrCity(e.target.value)}
                className="h-8 text-xs"
              />
            </div>
            <div>
              <Label className="text-xs">Postal Code</Label>
              <Input
                value={patchAddrPostal}
                onChange={(e) => setPatchAddrPostal(e.target.value)}
                className="h-8 text-xs"
              />
            </div>
            <div>
              <Label className="text-xs">Country</Label>
              <Input
                value={patchAddrCountry}
                onChange={(e) => setPatchAddrCountry(e.target.value)}
                className="h-8 text-xs"
              />
            </div>
          </div>
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() =>
              apiRequest(`/api/v1/users/me/addresses/${addrId}`, { method: "DELETE" })
            )
          }
        />
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() =>
              apiRequest(`/api/v1/users/me/addresses/${addrId}/default`, { method: "PATCH" })
            )
          }
        />

        {/* Seller Profile */}
        <h2 className="mb-3 mt-8 text-lg font-semibold text-slate-700">Seller Profile</h2>

        <EndpointCard
          method="GET"
          path="/api/v1/users/me/seller-profile"
          auth="AUTH / SELLER"
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() =>
              apiRequest("/api/v1/users/me/seller-profile", {
                method: "PATCH",
                body: JSON.stringify({
        ...
        {/* Seller Value Profile */}
        <h2 className="mb-3 mt-8 text-lg font-semibold text-slate-700">Seller Value Profile</h2>

        <EndpointCard
          method="GET"
          path="/api/v1/users/me/seller/value-profile"
          auth="AUTH / SELLER"
        ...
        <EndpointCard
          ...
          onExecute={() =>
            withToken(() => {
              let certs
              try {
                certs = JSON.parse(certifications)
              } catch {
                certs = []
              }
        ...
        >
          <div className="space-y-2">
            ...
            <div>
              <Label className="text-xs">Certifications (JSON array)</Label>
              <Textarea
