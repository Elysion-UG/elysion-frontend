      ...
      <tr className="transition-colors hover:bg-slate-800/30">
        ...
        <td className="px-4 py-3">
          <div className="flex items-center" style={{ paddingLeft: `${depth * 24}px` }}>
            ...
            <span className={`font-medium ${isActive ? "text-slate-200" : "text-slate-500 line-through"}`}>
              {node.name}
            </span>
          </div>
        </td>
        ...
        {/* Level badge */}
        <td className="px-4 py-3">
          <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${levelColor[node.level] ?? ""}`}>
