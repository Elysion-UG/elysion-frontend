...
export default function ErrorTrendChart({ events, hours }: ErrorTrendChartProps) {
  ...
  if (events.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center rounded-xl border border-border bg-ink-900/60 p-6">
        <p className="text-sm text-muted-foreground">Keine Fehler im ausgewählten Zeitraum.</p>
      </div>
    )
  }

  return (
    <div className="rounded-xl border border-border bg-ink-900/60 p-6">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#34403A" />
          <XAxis
            dataKey="time"
            stroke="#5D6B63"
            tick={{ fontSize: 11, fill: "#5D6B63" }}
            interval="preserveStartEnd"
          />
          <YAxis stroke="#5D6B63" tick={{ fontSize: 11, fill: "#5D6B63" }} allowDecimals={false} />
          <Tooltip
            contentStyle={{
            ...
            labelStyle={{ color: "#F4EEDF" }}
          />
          <Legend wrapperStyle={{ fontSize: "12px", color: "#F4EEDF" }} />
          <Line
            type="monotone"
            dataKey="critical"
            name="Kritisch"
