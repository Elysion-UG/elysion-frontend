...
describe('AdminService', () => {
  ...
  it('listUsers — appends page, size, search, role, status query params', async () => {
    ...
    await AdminService.listUsers({ page: 1, pageSize: 10, search: 'alice', role: 'BUYER', status: 'ACTIVE' })
  ...
  it('getUser — calls GET /api/v1/admin/users/:id', async () => {
    const mockUser = { id: 'u1', email: 'a@b.com', role: 'BUYER', status: 'ACTIVE', emailVerified: true, createdAt: '' }
  ...
  it('rejectSellerProfile — calls POST /api/v1/admin/seller-profiles/:id/reject with reason', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ method: 'POST', body: JSON.stringify({ reason: 'Incomplete docs' }) })
    )
  })
  ...
  it('suspendSellerProfile — calls POST /api/v1/admin/seller-profiles/:id/suspend with reason', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ method: 'POST', body: JSON.stringify({ reason: 'Policy violation' }) })
    )
  })
  ...
  it('verifyCertificate — calls PATCH /api/v1/admin/certificates/:id/verify', async () => {
    mockApiRequest.mockResolvedValue({ id: 'cert-1', status: 'VERIFIED', verifiedByAdminId: 'admin-1', verifiedAt: '', rejectionReason: null })
  ...
  it('rejectCertificate — calls PATCH /api/v1/admin/certificates/:id/reject with reason', async () => {
    mockApiRequest.mockResolvedValue({ id: 'cert-1', status: 'REJECTED', verifiedByAdminId: null, verifiedAt: null, rejectionReason: 'Expired' })
  ...
  it('listOrders — appends page, size, status params', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      '/api/v1/admin/orders?page=1&size=25&status=PAID'
    )
  })
